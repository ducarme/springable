...
def compute_requirements_for_animation(mdl: dynamics.DynamicModel, t, q, dqdt, f):
    ...
    for i in range(q.shape[0]):
        _assembly.set_general_coordinates(q[i, :])
        _assembly.set_general_velocities(dqdt[i, :])
    ...
    _assembly.set_general_coordinates(q[0, :])
    _assembly.set_general_velocities(dqdt[0, :])
