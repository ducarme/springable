...
class DynamicSolver:
    ...
    def __init__(self, mdl: dynamics.DynamicModel):
        ...
        self._initial_positions = self._model.get_assembly().get_coordinates()
    ...
    def _solve_with_runge_kutta4(self, t_max=10.0, rtol=1e-5, atol=1e-5, verbose=True):
        ...
        self._model.get_assembly().set_coordinates(self._initial_positions)
        self._model.get_assembly().set_velocities(self._initial_velocities)
    ...
    def _ode_system(self, time, state):
        ...
        self._model.get_assembly().set_coordinates(self.to_structural_coordinates(q))
        self._model.get_assembly().set_velocities(self.to_structural_velocities(dqdt))

        # compute forces from model
        internal_force_vector = self._model.get_assembly().compute_elastic_force_vector()[self._free_dof_indices]
